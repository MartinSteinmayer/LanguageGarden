...
const MapboxExample = () => {
    ...
    // Load all language data (voice available, endangered, severely endangered, descriptions)
    useEffect(() => {
    ...
    useEffect(() => {
        if (!countriesData || !descriptionsData) return; // Wait for all data to load

        mapboxgl.accessToken =
            "pk.eyJ1IjoibWFydGluc3RlaW5tYXllciIsImEiOiJjbHduem5pdnExMW9jMnFueGFmcHliaWczIn0.jjbMWXUB0w-rbkPMFbiz5Q";
        ...
        // Helper function to get language description from loaded data
        function getLanguageDescription(fullName, dialectKey, baseName) {
            ...
            console.log(`No description found for: "${fullName}"`);
            // If no description found, return a default message
            return `No description available for ${fullName}`;
        }
        ...
        map.on("style.load", () => {
            ...
            map.addLayer({
                ...
                paint: {
                    ...
                    "circle-blur": ["case", ["boolean", ["feature-state", "hover"], false], 0.2, 0],
                },
                layout: {
    ...
    }, [countriesData, descriptionsData]); // Depend on both data sources
    ...
    return (
        <div className="relative w-full h-screen">
            ...
            {/* Loading State */}
            {(!countriesData || !descriptionsData) && (
            ...
            {selectedLanguage && (
                <div className="absolute top-6 right-6 z-10">
                    <LanguageInfoCard languageGroup={selectedLanguage} onClose={() => setSelectedLanguage(null)} />
                </div>
            )}

            {/* Map Legend */}
            <div className="absolute bottom-4 left-1 md:bottom-6 md:left-6 z-10 scale-75 md:scale-100">
                <MapLegend />
            </div>

            {/* Header */}
            <div className="hidden md:block absolute top-6 left-6 z-10">
                <div className="bg-gradient-to-r from-green-50 to-green-100 backdrop-blur-sm rounded-lg px-6 py-4 shadow-lg">
            ...
            {!selectedLanguage && !hoveredLanguage && (
                ...
                >
                    <div
                        className="bg-black/80 text-white px-2 py-1 rounded-lg text-xs"
                        style={{
